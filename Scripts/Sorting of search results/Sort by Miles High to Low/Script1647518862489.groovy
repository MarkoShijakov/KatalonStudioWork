...
WebUI.click(findTestObject('Search cars Page/Sort by/Miles (High to Low)'))

WebDriver driver = DriverFactory.getWebDriver()

ArrayList<WebElement> miles = driver.findElements(By.cssSelector('.odometer-reading'))

int oldMiles = 1000009

for (int i = 0; i < 10; i++) {
    String milesString = miles.get(i).getText().replaceAll('[,]', '')

    int newMiles = Integer.parseInt(milesString)

    WebUI.verifyGreaterThanOrEqual(oldMiles, newMiles)

    //moze da ide i less than or equal ali se onda prvo pise nova pa onda stara vrednost
    System.out.println(((((('\n' + 'Previous Car Miles: ') + oldMiles) + '\n') + 'Next Car Miles: ') + newMiles) + '\n')
